...
- After 1430Hrs, we can check SG in NLDC website, if at that time, it is found that we got if any block below [[technical minimum]] SG, and we have more than 8 blocks technical minimum SG for more than 8 blocks in [[Peak hours]], then it is our right to ask NLDC to ask for above technical minimum for all blocks in a day
...
- After 1430Hrs, except for the units that got [[SCUC]], for all other generators, their respective beneficiaries can change the SG whenever and how much ever they want, they can simply make 0 also
...
And may refer [[Real Time Scheduling]], for real time scheduling